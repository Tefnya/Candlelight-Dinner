...
public class CandlelightLeggingsExtensions implements IClientItemExtensions {
    @Override
    public @NotNull Model getGenericArmorModel(@NotNull LivingEntity entity, @NotNull ItemStack stack, @NotNull EquipmentSlot slot, @NotNull HumanoidModel<?> original) {
        if (slot != EquipmentSlot.LEGS || !(stack.getItem() instanceof CandlelightLegsItem legs)) return original;
        return ArmorRegistry.getLeggingsModel(legs, original.rightLeg, original.leftLeg, original);
    }
}